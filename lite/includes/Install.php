...
use KaizenCoders\Logify\Option;
...
class Install {
	...
	private static $db_updates = [
		'1.0.0' => [
			'kaizencoders_logify_update_100_db_version',
		],
	];
	...
	public static function install() {
		...
		if ( self::is_new_install() ) {
			...
			Helper::maybe_define_constant( 'KAIZENCODERS_LOGIFY_INSTALLING', true );
	...
	private static function maybe_update_db_version() {
		if ( self::needs_db_update() ) {
			if ( apply_filters( 'kaizencoders_logify_enable_auto_update_db', true ) ) {
				self::update();
			}
		}
	}
	...
	public static function get_100_schema() {
		global $wpdb;
		...
		$tables = "
            CREATE TABLE `{$wpdb->prefix}kc_lf_activity_logs` (
				`id` int(10) NOT NULL AUTO_INCREMENT,
				`site_id` bigint(20) DEFAULT 1,
				`event_id` bigint(20) NOT NULL,
				`object` varchar(255) NOT NULL,
				`event_type` varchar(255) NOT NULL,
				`severity` bigint(20) DEFAULT 0,
